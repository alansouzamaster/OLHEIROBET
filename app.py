...
def formatar_metric(label, prob):
    ...
    <div style='margin-bottom: 18px;'>
        ...
        <div style='background-color: #2d333b; border-radius: 10px; height: 8px; width: 100%;'>
            <div style='background-color: {cor}; width: {prob}%; height: 100%; border-radius: 10px; {glow}'></div>
        </div>
    </div>
    """

# --- INTERFACE (CONFIG E CSS) ---
st.set_page_config(page_title="PROBET v6.0", layout="wide")
...
st.markdown("""
    ...
    .card-gols { background: linear-gradient(145deg, #16222e, #0f171f); padding: 22px; border-radius: 15px; border: 1px solid #1e3a5f; margin-bottom: 20px;}
    .card-cantos { background: linear-gradient(145deg, #162e21, #0f1f16); padding: 22px; border-radius: 15px; border: 1px solid #1e5f3a; margin-bottom: 20px;}
    .card-cards { background: linear-gradient(145deg, #2e2616, #1f1a0f); padding: 22px; border-radius: 15px; border: 1px solid #5f4d1e; margin-bottom: 20px;}

    .section-header { font-size: 16px; font-weight: bold; color: #ffc107; margin-bottom: 15px; text-align: center; text-transform: uppercase; letter-spacing: 1.5px; }
...
if st.session_state.analise_pronta and st.session_state.jogo_selecionado:
    ...
    with col1:
        ...
        # Unindo as métricas antes do markdown para evitar erro de texto exposto
        html_gols = formatar_metric("OVER 1.5 GOLS", calcular_poisson(m_total, 1))
        html_gols += formatar_metric("OVER 2.5 GOLS", calcular_poisson(m_total, 2))
        st.markdown(f"<div class='card-gols'>{html_gols}</div>", unsafe_allow_html=True)

    with col2:
        st.markdown("<div class='section-header'>🚩 Cantos</div>", unsafe_allow_html=True)
        html_cantos = formatar_metric("OVER 5.5 CANTOS", calcular_poisson(9.5, 5))
        html_cantos += formatar_metric("OVER 8.5 CANTOS", calcular_poisson(9.5, 8))
        st.markdown(f"<div class='card-cantos'>{html_cantos}</div>", unsafe_allow_html=True)

    with col3:
        st.markdown("<div class='section-header'>🟨 Cartões</div>", unsafe_allow_html=True)
        html_cards = formatar_metric("OVER 1.5 CARTÕES", calcular_poisson(4.2, 1))
        html_cards += formatar_metric("OVER 3.5 CARTÕES", calcular_poisson(4.2, 3))
        st.markdown(f"<div class='card-cards'>{html_cards}</div>", unsafe_allow_html=True)
